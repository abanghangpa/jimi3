#!/usr/bin/env python3
"""
Sweep Detector v3 — Forward-fill OI, proper alignment.
"""
...
def load_ohlcv(path, start="2026-04-13"):
    bars = []
    with open(path) as f:
...
def load_oi(path):
    """Load OI as sorted (datetime, value) list."""
    data = []
    with open(path) as f:
        ...
        for row in reader:
            ...
            if not ts or not oi_str: continue
            try:
                clean = ts.replace("T", " ")[:19]
                data.append((datetime.strptime(clean, "%Y-%m-%d %H:%M:%S"), float(oi_str)))
            except: continue
    return sorted(data, key=lambda x: x[0])

def build_oi_lookup(oi_data):
    """Build forward-fill lookup: for each OI timestamp, store value."""
    return {dt: val for dt, val in oi_data}

def get_oi_forward_fill(oi_sorted, bar_ts_str):
    """Get OI using forward-fill: last known OI at or before bar time."""
    try:
        bar_dt = datetime.strptime(bar_ts_str[:19], "%Y-%m-%d %H:%M:%S")
    except:
        return None

    # Binary search for last OI <= bar_dt
    lo, hi = 0, len(oi_sorted) - 1
    result = None
    while lo <= hi:
        mid = (lo + hi) // 2
        if oi_sorted[mid][0] <= bar_dt:
            result = oi_sorted[mid][1]
            lo = mid + 1
        else:
            hi = mid - 1
    return result
...
def find_significant_levels(bars, idx, lookback=192):
    """S/R with 2+ touches or session H/L."""
    ...
    for lv in sorted(levels, key=lambda x: x["strength"], reverse=True):
        ...
        for d in deduped:
            if abs(lv["price"] - d["price"]) / d["price"] < tol:
                ...
                found = True
                break
        if not found: deduped.append(lv)
    return sorted(deduped, key=lambda x: x["strength"], reverse=True)[:10]


def detect_sweep(bars, oi_sorted, idx, sr_levels, atr):
    """Detect sweep with strict criteria + forward-fill OI."""
    bar = bars[idx]
    ...
    # OI via forward-fill
    oi_now = get_oi_forward_fill(oi_sorted, bar["ts"])
    oi_prev = get_oi_forward_fill(oi_sorted, bars[idx-1]["ts"])
    if oi_now is None or oi_prev is None or oi_prev == 0:
        return None
    oi_change = (oi_now - oi_prev) / oi_prev

    if oi_change >= -0.003: return None  # Need 0.3%+ drop
    if vol < avg_vol * 1.3: return None
...
def run_backtest(bars, oi_sorted, regimes):
    trades = []; positions = []
    ...
    for i, bar in enumerate(bars):
        ...
        for pos, outcome, pnl in closed:
            ...
            trades.append({
                "strategy": "liquidity_grab_v3", "direction": pos["direction"],
        ...
        sweep = detect_sweep(bars, oi_sorted, i, sr, atr)
...
if __name__ == "__main__":
    print("=" * 80)
    print("SWEEP DETECTOR v3 — Forward-Fill OI")
    print("=" * 80)

    bars = load_ohlcv(os.path.join(BASE, "data", "eth_15m_merged.csv"))
    oi_data = load_oi(os.path.join(BASE, "data", "derivatives_history", "derivatives_collected.csv"))

    print(f"\n  OHLCV: {len(bars)} bars ({bars[0]['ts']} -> {bars[-1]['ts']})")
    print(f"  OI: {len(oi_data)} points ({oi_data[0][0]} -> {oi_data[-1][0]})")

    # Test forward-fill matching
    match = 0
    test_bars = [b for b in bars if b["ts"] >= "2026-06-15" and b["ts"] < "2026-06-16"]
    for bar in test_bars[:10]:
        oi = get_oi_forward_fill(oi_data, bar["ts"])
        if oi is not None: match += 1
    print(f"  OI forward-fill test (Jun 15, 10 bars): {match}/10")
    ...
    out_path = os.path.join(BASE, "data", "5agent_backtest", "sweep_v3.json")
