...
# Ensure all subsequent operations use the new index
...
def test_strat(df, mask, name, label):
    ...
    # Align index
    shifted = shifted.reindex(df.index).fillna(False)
    events = df[shifted]
...
def detect_judas(df):
    idx = df.index
    highs = pd.Series(df['High'].values.astype(float), index=idx)
    closes = pd.Series(df['Close'].values.astype(float), index=idx)
    volumes = pd.Series(df['Volume'].values.astype(float), index=idx)
    vol_ma = volumes.rolling(20).mean()
    vol_ok = volumes >= vol_ma
    daily_highs = highs.rolling(96).max().shift(1)
    session_highs = highs.rolling(32).max().shift(1)
    wick_up = highs - closes
    body = (closes - closes.shift(1)).abs().replace(0, 0.001)
    sweep_d = (highs > daily_highs * 1.001) & (closes < daily_highs) & (wick_up > body * 1.5) & vol_ok
    sweep_s = (highs > session_highs * 1.001) & (closes < session_highs) & (wick_up > body * 1.5) & vol_ok
