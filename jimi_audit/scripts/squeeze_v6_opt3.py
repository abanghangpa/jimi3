#!/usr/bin/env python3
"""Squeeze V6 Optimizer — Fixed to match V6-A (4805 trades)."""
...
def generate_signals(closes, highs, lows, volumes, ind, n):
    """
    V6-A raw invert: every BB squeeze breakout = fade signal.
    No Keltner, no squeeze detection. Just BB width < 2% + price outside BB.
    """
    signals = []
    for i in range(60, n):
        price = closes[i]
        ...
        # Check if price is outside BB (breakout)
        ...
        signals.append({"bar": i, "direction": direction, "entry": price,
                        "atr": a, "bb_width": ind["bb_width"][i],
                        "vol_ratio": vol_ratio, "ema_50": ema_50})
    return signals
