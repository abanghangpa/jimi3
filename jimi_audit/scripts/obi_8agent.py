"""
8-Agent Protocol: orderbook_imbalance isolation gate.
Tests OBI detection: bid/ask imbalance + volume + directional bias.
"""
import pandas as pd
import numpy as np
...
# Read the strategy source to understand detection logic
print("Strategy: orderbook_imbalance (s19)")
print("Detection: bid/ask imbalance from orderbook snapshots + volume + taker flow")
print("Data needed: L2 orderbook (bid/ask depth), taker buy/sell volume")
print("Look-ahead bias: Uses current-bar data only (no future lookback)")
print("OHLCV: clean, no gaps")
print(f"EMA200 coverage: {merged['ema200'].notna().sum()}/{len(merged)}")

# ═══════════════════════════════════════════════════════
# AGENT 2: NON-INDICATOR — Test raw taker imbalance
# ═══════════════════════════════════════════════════════
print("\n" + "="*70)
print("AGENT 2: NON-INDICATOR — Raw taker imbalance detection")
print("="*70)

# OBI detection proxies (without L2 orderbook data):
# 1. Taker buy ratio (buy_vol / total_vol) — measures aggressive buying
# 2. Volume spike + direction
# 3. Price momentum + volume confirmation

# Detect taker imbalance events
taker_imbalance = merged['taker_ratio']

# Extreme taker events (>0.65 = aggressive buying, <0.35 = aggressive selling)
taker_buy_extreme = taker_imbalance > 0.65
taker_sell_extreme = taker_imbalance < 0.35

# Volume spike (above average)
vol_spike = merged['vol_ratio'] > 1.2

# Combine: taker extreme + volume spike
buy_mask = (taker_buy_extreme & vol_spike).shift(1).fillna(False)
sell_mask = (taker_sell_extreme & vol_spike).shift(1).fillna(False)
all_mask = buy_mask | sell_mask

buy_events = merged[buy_mask]
sell_events = merged[sell_mask]
all_events = merged[all_mask]

print(f"\nTaker buy extreme (>0.65) + vol spike: {len(buy_events)} events")
print(f"Taker sell extreme (<0.35) + vol spike: {len(sell_events)} events")
print(f"Total events: {len(all_events)}")

# Also test with different thresholds
for thresh_buy, thresh_sell in [(0.60, 0.40), (0.65, 0.35), (0.70, 0.30), (0.75, 0.25)]:
    m = ((taker_imbalance > thresh_buy) | (taker_imbalance < thresh_sell)) & vol_spike
    m = m.shift(1).fillna(False)
    n = m.sum()
    rets = merged.loc[m, 'fwd_ret_16'].dropna()
    if len(rets) < 5:
        continue
    mean_r = rets.mean()
    t, p = stats.ttest_1samp(rets, 0)
    gate = "PASS" if p < 0.1 and mean_r > 0.001 else "FAIL"
    print(f"  {'+' if gate=='PASS' else '-'} taker>{thresh_buy} or <{thresh_sell} + vol>1.2: n={n:5d}, mean={mean_r*100:+.4f}%, p={p:.4f} [{gate}]")
...
for direction, events in [('BUY (LONG)', buy_events), ('SELL (SHORT)', sell_events), ('ALL', all_events)]:
...
configs = [
    ...
    ('taker>0.65, vol>1.2 (default)', 0.65, 1.2),
    ('taker>0.70, vol>1.2', 0.70, 1.2),
    ('taker>0.70, vol>1.5', 0.70, 1.5),
    ('taker>0.75, vol>1.5', 0.75, 1.5),
    ('taker>0.75, vol>2.0', 0.75, 2.0),
    ('taker>0.80, vol>2.0', 0.80, 2.0),
]

for name, taker_thresh, vol_thresh in configs:
    vol_ok = merged['vol_ratio'] > vol_thresh
    buy_m = (taker_imbalance > taker_thresh) & vol_ok
    sell_m = (taker_imbalance < (1 - taker_thresh)) & vol_ok
    mask = (buy_m | sell_m).shift(1).fillna(False)
    events = merged[mask]
...
filters = [
    ...
    ('+ taker > 0.70', taker_imbalance > 0.70),
    ('+ taker > 0.75', taker_imbalance > 0.75),
...
# Test alternative imbalance detection methods
alt_configs = [
    ('Volume delta (buy-sell)', (pd.Series(taker_base) - (pd.Series(volumes) - pd.Series(taker_base))).abs() > pd.Series(volumes) * 0.3),
    ('Taker ratio momentum (4-bar)', taker_imbalance.diff(4).abs() > 0.15),
    ('Volume spike only (no direction)', merged['vol_ratio'] > 2.0),
    ('Taker + price momentum', (taker_imbalance > 0.65) & (pd.Series(closes).pct_change(4) > 0.005)),
    ('Taker + price down', (taker_imbalance > 0.65) & (pd.Series(closes).pct_change(4) < -0.005)),
    ('Taker divergence (ratio up, price down)', (taker_imbalance > 0.60) & (pd.Series(closes).pct_change(4) < -0.003)),
]
...
print(f"Gate (existing): PASS — 847 events, +0.254%, p=0.001")
